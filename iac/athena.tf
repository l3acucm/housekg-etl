...
# Athena named query for prediction fact
resource "aws_athena_named_query" "prediction_fact_query" {
  name        = "prediction_fact_query"
  workgroup   = aws_athena_workgroup.housekg_workgroup.name
  database    = aws_glue_catalog_database.data_db.name
  description = "Query for prediction fact data"
  query       = "SELECT * FROM ${aws_glue_catalog_database.data_db.name}.prediction_fact LIMIT 100;"
}
