...
# Keyword-flagged candidates (basement) are cheap yes/no Haiku calls, not the more
# involved anomaly correction — cap separately and higher; a batch can easily have
# 100+ genuine "цоколь" mentions and MAX_LLM_CALLS is tuned for the anomaly-
# correction path.
...
def main():
    ...
    realty_dim_df = scored_df.drop("sqm_price", "expected_price", "price_vs_expected_pct")
    ...
    qualifying_slugs = [
        r["slug"] for r in
        new_rows.filter((F.col("square") > 60) & (F.col("sqm_price") * F.col("square") < 120000))
        .select("slug").collect()
    ]
